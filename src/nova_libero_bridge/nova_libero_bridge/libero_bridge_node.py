...
# LIBERO 动作向量:7 维 OSC_POSE (dx, dy, dz, droll, dpitch, dyaw, gripper)
ACTION_DIM = 7


def zero_action_vector() -> list[float]:
...
class LiberoBridgeNode(Node):
    def __init__(self) -> None:
        super().__init__("libero_bridge")
        ...
        self.get_logger().info(
            f"Using LIBERO sim server at {self.server_host}:{self.server_port}"
        )
        self.obs: dict[str, Any] | None = None
        self.latest_action = zero_action_vector()
        ...
        self.create_subscription(
            Float32MultiArray,
            "/nova/libero/action_cmd",
            self.action_callback,
            10,
        )
        self.state_pub = self.create_publisher(String, "/nova/libero/state", 10)
        self.reward_pub = self.create_publisher(Float32, "/nova/libero/reward", 10)
        self.success_pub = self.create_publisher(Bool, "/nova/libero/success", 10)

        self.create_service(Trigger, "/nova/libero/reset", self.reset_callback)
        self.create_service(Trigger, "/nova/libero/step_zero", self.step_zero_callback)
    ...
    def _reset_env(self) -> None:
        ...
        self.get_logger().info(
            f"LIBERO env reset (benchmark={self.benchmark} task_id={self.task_id})"
        )
    ...
    def _ensure_camera_publishers(self, obs: dict[str, Any]) -> None:
        for key, value in obs.items():
            ...
            topic = f"/nova/libero/cameras/{topic_suffix}/image_raw"
    ...
    def action_callback(self, msg: Float32MultiArray) -> None:
        try:
            ...
            if values.size != ACTION_DIM:
                raise ValueError(
                    f"expected {ACTION_DIM} action values, got {values.size}"
                )
            self.latest_action = values.tolist()
