"""AgentOS 的短期 session 与任务记忆(含 JSON 文件持久化)。"""
...
def _now() -> str:
    """返回当前 UTC 时间字符串(格式 %Y-%m-%dT%H:%M:%SZ)。"""
    return time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())


def _write_json_atomic(path: Path, value: dict) -> None:
    """把 JSON 先写入同目录临时文件并 fsync,再原子替换目标文件,避免写坏。"""
...
class TaskMemory:
    ...
    def from_dict(cls, data: dict) -> "TaskMemory":
        """从持久化字典恢复任务记忆对象,并根据 started_at 还原计时基准。"""
        task = cls(
...
class SessionManager:
    """管理 session 生命周期及其 JSON 文件持久化,所有写操作走原子替换。"""
    ...
    def _load_record(self, session_id: str) -> SessionRecord:
        """从 session.json 与 context.json 加载会话记录;不存在或损坏时抛异常。"""
    ...
    def get(self, session_id: str, allow_ended: bool = False) -> SessionRecord:
        """获取 session;allow_ended=False 时非 active 会抛 ValueError。"""
        with self._lock:
            record = self._load_record(session_id)
...
class Compactor:
    """上下文压缩器:按 token 预算保留近期任务详情,较早任务只留摘要。"""
    ...
    def compact(self, tasks: list[TaskMemory], current: TaskMemory) -> tuple[list[dict], bool]:
        """按 token 预算保留近期任务详情、压缩较早任务为摘要。

        返回 (上下文载荷, 是否发生了压缩)。超出预算时优先丢弃较早的摘要。
        """
...
class ContextBuilder:
    """把结构化记忆组装成 LLM messages,并在超预算时触发压缩。"""
    ...
    def build(
        ...
        """组装 messages:system 提示 + session 历史/当前任务/skill 索引 + 观测 + 工具 schema。"""
